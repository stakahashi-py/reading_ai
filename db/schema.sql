...
-- vector column for books (768 dims)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name='books' AND column_name='embed'
  ) THEN
    EXECUTE 'ALTER TABLE books ADD COLUMN embed vector(768)';
  END IF;
END
$$ LANGUAGE plpgsql;
